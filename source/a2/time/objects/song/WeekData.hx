...
class WeekData 
{
	...
	public static function reloadWeekFiles(isStoryMode:Null<Bool> = false)
	{
		...
		var directories:Array<String> = ['mods/', Paths.getPreloadPath()];
		...
		if(FileSystem.exists(modsListPath))
		{
			...
			for (i in 0...stuff.length)
			{
				...
				{
					var path = haxe.io.Path.join(['mods/', splitName[0]]);
					//trace('trying to push: ' + splitName[0]);
					if (sys.FileSystem.isDirectory(path) && !Paths.ignoremods.contains(splitName[0]) && !disabledMods.contains(splitName[0]) && !directories.contains(path + '/'))
					{
		...
		{
			var pathThing:String = haxe.io.Path.join(['mods/', folder]) + '/';
			...
			for (j in 0...directories.length) 
			{
				...
				if(!weeksLoaded.exists(sexList[i])) 
				{
					...
					if(week != null) {
						...
						if(j >= originalLength)
							weekFile.folder = directories[j].substring('mods/'.length, directories[j].length-1);
	...
	private static function addWeek(weekToCheck:String, path:String, directory:String, i:Int, originalLength:Int)
	{
		if(!weeksLoaded.exists(weekToCheck))
		{
			...
			if(week != null)
			{
				...
				if(i >= originalLength)
					weekFile.folder = directory.substring('mods/'.length, directory.length-1);
