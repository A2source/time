...
/**
 * This class extends `FlxSkewedSprite` to display video files in HaxeFlixel.
 *
 * ```haxe
 ...
 */
@:nullSafety
class FlxVideoSprite extends FlxSkewedSprite
{
	 ...
	 * @return The `FlxVideoSprite` object.
	 */
	public function load(location:Location, ?options:Array<String>):FlxVideoSprite
	{
		if (bitmap == null) return this;

		if (location is String)
		{
			@:privateAccess var cache:FlxInternalVideo = Assets.videoCache.get(location);
			if (cache == null) bitmap.load(location, options);
			else bitmap = cast cache;
		}
		else
			bitmap.load(location, options);
